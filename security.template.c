...
static void sha256_block(const uint8_t block[64], uint32_t state[8]) {
    ...
    for (uint32_t index = 0; index < 64; ++index) {
        ...
        uint32_t t1 = h + s1 + choose + round_constants[index] + schedule[index];
...
void nbu_password_hash(const char *password, uint8_t digest[32]) {
    ...
    uint32_t length = 0;
    while (password[length] && length < 55) { block[length] = (uint8_t)password[length]; ++length; }
    block[length] = 0x80; block[63] = (uint8_t)(length * 8);
