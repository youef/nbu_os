...
mkdir -p "$ROOTFS" "$MNT"
apt-get update
DEBIAN_FRONTEND=noninteractive apt-get install -y debootstrap grub-pc-bin grub-common parted e2fsprogs dosfstools
...
apt-get update
apt-get install -y linux-image-amd64 grub-pc grub-pc-bin grub-common systemd-sysv sudo dbus-x11 xorg openbox xterm firefox-esr network-manager network-manager-gnome fonts-dejavu fonts-noto-core ca-certificates curl
...
(sleep 3; firefox-esr --safe-mode --kiosk --private-window "https://www.google.com") &
AUTO
